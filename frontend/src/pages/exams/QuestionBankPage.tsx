...
const BLUEPRINTS: Record<string, any[]> = {
  ...
  VSTEP_4_SKILLS: [
    ...
    { skill: 'Writing', part: 'Writing Task 1', label: 'Writing Task 1 - Viết thư/Email', type: 'writing_email_letter', section: 1200, minWords: 120, score: 10, email: true, note: 'Tối thiểu 120 từ, chiếm 1/3 điểm Writing' },
    { skill: 'Writing', part: 'Writing Task 2', label: 'Writing Task 2 - Viết bài luận', type: 'writing_essay', section: 2400, minWords: 250, score: 10, note: 'Tối thiểu 250 từ, chiếm 2/3 điểm Writing' },
    { skill: 'Speaking', part: 'Speaking Part 1', label: 'Speaking Phần 1 - Tương tác xã hội', type: 'speaking_social_interaction', group: true, audio: true, section: 180, score: 10, note: 'Nhập 2 chủ đề, mỗi chủ đề 3 câu hỏi; ghi âm liên tục' },
    { skill: 'Speaking', part: 'Speaking Part 2', label: 'Speaking Phần 2 - Thảo luận giải pháp', type: 'speaking_solution_discussion', group: true, prep: 60, response: 180, score: 10, note: 'Tổng 4 phút: 60 giây chuẩn bị + 3 phút nói' },
    { skill: 'Speaking', part: 'Speaking Part 3', label: 'Speaking Phần 3 - Phát triển chủ đề', type: 'speaking_topic_development', group: true, image: true, prep: 60, response: 240, score: 10, note: 'Tổng 5 phút: 60 giây chuẩn bị + 4 phút nói' },
  ],
};
...
function QuestionForm({ initial, onClose, onSuccess }: { initial?: any; onClose: () => void; onSuccess: () => void }) {
  ...
  const [form, setForm] = useState<any>({
    ...
    score: initial?.score || initialRule.score || 1,
  ...
  const changeRule = (part: string) => {
    ...
    setForm((f: any) => ({ ...f, skill: next.skill, part: next.part, questionType: next.type, score: next.score || 1, options: makeOptions(next.options || []), prepSeconds: next.prep || '', responseSeconds: next.response || '', sectionSeconds: next.section || '', minWords: next.minWords || '', groupKey: next.group ? '' : '' }));
  };
  const changeFormat = (formatCode: string) => {
    const next = BLUEPRINTS[formatCode][0];
    setForm((f: any) => ({ ...f, formatCode, examType: formatCode === 'VSTEP_4_SKILLS' ? 'VSTEP' : 'TOEIC', skill: next.skill, part: next.part, questionType: next.type, score: next.score || 1, options: makeOptions(next.options || []), prepSeconds: next.prep || '', responseSeconds: next.response || '', sectionSeconds: next.section || '', minWords: next.minWords || '', groupKey: next.group ? '' : '' }));
  };
